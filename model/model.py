...
class UserData(BaseModel):
    ...
    _message_fields_schema = ('created', 'id', 'auth_provider',
                              'name', 'email')

    auth_provider = ndb.StringProperty(
        choices=[PROVIDER_GOOGLE, PROVIDER_FACEBOOK], required=True)
...
class MeetingRequest(BaseModel):
    '''Class to represent user requests to join a specific meeting
    In this entity, users are part of the ancestor path'''
    ...
    def ParentMeetingSet(self, value):
        ...
        # Add the key to the query info. Scenario: fetch entities for meeting
